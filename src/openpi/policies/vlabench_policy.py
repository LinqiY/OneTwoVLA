...
class VLABenchInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        mask_padding = self.model_type == _model.ModelType.PI0  # We don't mask for pi0-FAST / pi0-FAST-FUSE.
        ...
        # Pi0Fuse / Pi0FASTFuse (OneTwoVLA): FuseTokenize needs thought + flags; mirror UMIDataset direct-action mode.
        if self.model_type in (_model.ModelType.PI0_FUSE, _model.ModelType.PI0_FAST_FUSE) and "prompt" in data:
            p = data["prompt"]
            if not isinstance(p, str):
                p = p.item() if hasattr(p, "item") else str(p)
            inputs["thought"] = [p.strip()]
            inputs["act_with_outdated_thought"] = False
            inputs["think_with_outdated_thought"] = False
