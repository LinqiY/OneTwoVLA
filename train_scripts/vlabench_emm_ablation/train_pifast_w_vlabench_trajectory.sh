...
# XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 python scripts/train_cotrain.py pifast_w_vlabench_relative_cotrain_trajectory \
#   --exp-name="${now_date}/${now_seconds}/pifast-w-vlabench-cotrain-mm-data" \
#   --batch-size="${batch_size}" \
#   --save_interval=10000
XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 python scripts/train_cotrain.py pifast_w_vlabench_relative_cotrain_trajectory \
  --exp-name="2026.05.15/08.45.02/pifast-w-vlabench-cotrain-mm-data-libero" \
  --batch-size="${batch_size}" \
  --save_interval=10000 \
  --resume
