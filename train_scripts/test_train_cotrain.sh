...
# python scripts/smoke_test_cotrain.py --config pifast_vlabench_cotrain --only-data
XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 python scripts/smoke_test_cotrain.py --config pifast_vlabench_cotrain --num-steps 100