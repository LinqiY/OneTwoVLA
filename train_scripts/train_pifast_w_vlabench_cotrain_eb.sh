#!/usr/bin/env bash
# VLA + VL (VLABench primitive JSONs). PaliGemma from VLABench swift→jax npz.
# Config: pifast_w_vlabench_cotrain_eb
set -euo pipefail
...
XLA_PYTHON_CLIENT_MEM_FRACTION=0.95 python scripts/train_cotrain.py pifast_w_vlabench_cotrain_eb \
  --exp-name="${now_date}/${now_seconds}/pifast-w-vlabench-cotrain-eb" \
